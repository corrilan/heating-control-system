...
RAW=`$KHAL_PATH list --once --format "{start-time} {end-time} {title}" | tail -n2`

echo "Trying to retrieve heating schedule..."
echo $RAW

# If no on time is defined on the Synology NAS Calendar, or if Synology NAS was unobtainable, then set a dummy schedule so heating remains off and under manual control, then quit the script
...
while read -r line; do
	...
	echo Off time 24 H $OFF_TIME_24HR_H
